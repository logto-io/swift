...
extension LogtoClientTests {
    func testFetchOidcConfigOk() throws {
        let client = LogtoClient(useConfig: try LogtoConfig(endpoint: "/oidc_config:good", clientId: "foo"), session: NetworkSessionMock.shared)
    ...
    func testFetchOidcConfigFailed() throws {
        let client = LogtoClient(useConfig: try LogtoConfig(endpoint: "/oidc_config:bad", clientId: "foo"), session: NetworkSessionMock.shared)
