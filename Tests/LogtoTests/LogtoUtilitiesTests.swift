...
final class LogtoUtilitiesTests: XCTestCase {
    ...
    func testVerifyIdTokenFailure() throws {
        let idToken = IdTokenFixtures.rsaIdToken
        let jwk = try RSAPublicKey(data: Data(IdTokenFixtures.rsaJwkJson.utf8))
        ...
        let issuer = IdTokenFixtures.issuer
        let clientId = IdTokenFixtures.clientId
        ...
        XCTAssertThrowsError(try LogtoUtilities.verifyIdToken(
            ...
            clientId: clientId,
            jwks: jwks,
            forTimeInterval: IdTokenFixtures.expiresAt + LogtoUtilities.defaultIdTokenClockTolerance
        )) {
    ...
    func testVerifyIdTokenRSA() throws {
        ...
        XCTAssertNoThrow(try LogtoUtilities.verifyIdToken(
            ...
            clientId: clientId,
            jwks: jwks,
            forTimeInterval: IdTokenFixtures.issuedAt
        ))
    }

    func testVerifyIdTokenClockTolerance() throws {
        let jwks = try JWKSet(keys: [RSAPublicKey(data: Data(IdTokenFixtures.rsaJwkJson.utf8))])
        let tolerance = LogtoUtilities.defaultIdTokenClockTolerance
        ...
        func verify(
            ...
            try LogtoUtilities.verifyIdToken(
                IdTokenFixtures.rsaIdToken,
                issuer: IdTokenFixtures.issuer,
                clientId: IdTokenFixtures.clientId,
                jwks: jwks,
                clockTolerance: clockTolerance,
                forTimeInterval: time
            )
        }

        // `iat` on both edges of the default tolerance
        XCTAssertNoThrow(try verify(at: IdTokenFixtures.issuedAt - tolerance))
        XCTAssertNoThrow(try verify(at: IdTokenFixtures.issuedAt + tolerance))
        XCTAssertThrowsError(try verify(at: IdTokenFixtures.issuedAt - tolerance - 1)) {
            XCTAssertEqual($0 as? LogtoErrors.Verification, .jwtIssuedTimeIncorrect)
        }
        XCTAssertThrowsError(try verify(at: IdTokenFixtures.issuedAt + tolerance + 1)) {
            XCTAssertEqual($0 as? LogtoErrors.Verification, .jwtIssuedTimeIncorrect)
        }

        // `exp` on both edges of the default tolerance: right before the edge, only the `iat` check fails
        XCTAssertThrowsError(try verify(at: IdTokenFixtures.expiresAt + tolerance - 1)) {
            XCTAssertEqual($0 as? LogtoErrors.Verification, .jwtIssuedTimeIncorrect)
        }
        XCTAssertThrowsError(try verify(at: IdTokenFixtures.expiresAt + tolerance)) {
            XCTAssertEqual($0 as? LogtoErrors.Verification, .jwtExpired)
        }

        // An expired token passes when the tolerance also covers its lifetime
        let lifetime = IdTokenFixtures.expiresAt - IdTokenFixtures.issuedAt
        XCTAssertNoThrow(try verify(at: IdTokenFixtures.expiresAt + 100, clockTolerance: lifetime + 100))

        // A custom tolerance can be stricter or looser than the default
        XCTAssertThrowsError(try verify(at: IdTokenFixtures.issuedAt + 61, clockTolerance: 60)) {
            XCTAssertEqual($0 as? LogtoErrors.Verification, .jwtIssuedTimeIncorrect)
        }
        XCTAssertNoThrow(try verify(at: IdTokenFixtures.issuedAt + 61, clockTolerance: 600))
        XCTAssertThrowsError(try verify(at: IdTokenFixtures.issuedAt + 599)) {
            XCTAssertEqual($0 as? LogtoErrors.Verification, .jwtIssuedTimeIncorrect)
        }
        XCTAssertNoThrow(try verify(at: IdTokenFixtures.issuedAt + 599, clockTolerance: 600))
    }
